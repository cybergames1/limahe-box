...
+ (BOOL) _isSystemVertionLowIOS7
{
    static BOOL system_vertion_lower = NO;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (NSOrderedAscending == [[[UIDevice currentDevice] systemVersion] compare:@"7.0" options:NSNumericSearch]) {
            system_vertion_lower = YES;
        }
    });
    return system_vertion_lower;
}
...
- (void)setupLabels {
    ...
    label.numberOfLines = kTextMaxLineCount;
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    CGSize maxSize = CGSizeMake(maxWidth, (kLabelFontSize*1.4) * kTextMaxLineCount);
    CGSize labelSize = CGSizeZero;
    if ([MBProgressHUD _isSystemVertionLowIOS7]) {
        labelSize = [label.text sizeWithFont:label.font constrainedToSize:maxSize lineBreakMode:label.lineBreakMode];
    }
    else{
        labelSize = [label.text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:label.font} context:nil].size;
    }
