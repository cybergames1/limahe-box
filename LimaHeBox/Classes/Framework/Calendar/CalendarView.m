...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _defaultFont                = [UIFont fontWithName:@"HelveticaNeue" size:17.0f];
        _titleFont                  = [UIFont fontWithName:@"Helvetica-Bold" size:17.0f];
...
{
    DiamondView *diamond            = [[DiamondView alloc] initWithFrame:CGRectMake(frame.size.width/6, frame.size.height/6, 2*frame.size.width/3, 2*frame.size.height/3)];
    UIButton *button                = [UIButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.font          = _defaultFont;
    button.frame                    = frame;
    button.layer.borderColor        = _borderColor.CGColor;
    [button addSubview:diamond];
    [button sendSubviewToBack:diamond];
    [button     addTarget:self action:@selector(tappedDate:) forControlEvents:UIControlEventTouchUpInside];
    return button;
}

- (DiamondView *)diamondViewInButton:(UIButton *)button {
    
    for (UIView *v in button.subviews) {
        if ([v isKindOfClass:[DiamondView class]]) {
            return (DiamondView *)v;
        }
    }
    return nil;
}
...
-(void)configureDayButton:(UIButton *)button withDate:(NSDate*)date
{
    ...
    if([_selectedDate compare:date] == NSOrderedSame)
    {
        ...
        [[self diamondViewInButton:button] setHidden:NO];
    }
    else
    {
        // Unselected button
        ...
        [[self diamondViewInButton:button] setHidden:YES];
