...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i =0; i<[weekdays count]; i++) {
        ...
        if (i%[weekdays count] == 5 || i%[weekdays count] == 6) {
            CGContextSetFillColorWithColor(context,
                                           [UIColor colorWithRed:(72.0/255.0) green:(217.0/255.0) blue:(192.0/255.0) alpha:1.0].CGColor);
        }else {
            CGContextSetFillColorWithColor(context,
                                           [UIColor whiteColor].CGColor);
        }
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), 60, kVRGCalendarViewDayWidth+2, 20) withFont:font lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            CGContextBeginPath(context);
            CGContextMoveToPoint(context, targetX+kVRGCalendarViewDayWidth/2, targetY);
            CGContextAddLineToPoint(context,targetX,targetY+kVRGCalendarViewDayHeight/2);
            CGContextAddLineToPoint(context,targetX+kVRGCalendarViewDayWidth/2,targetY+kVRGCalendarViewDayHeight);
            CGContextAddLineToPoint(context,targetX+kVRGCalendarViewDayWidth, targetY+kVRGCalendarViewDayHeight/2);
...
//            CGContextFillPath(context);
        ...
        [date drawInRect:CGRectMake(targetX+2, targetY+kVRGCalendarViewDayHeight/2-17/2, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:@"HelveticaNeue" size:17] lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        int targetX = targetColumn * (kVRGCalendarViewDayWidth+2) + kVRGCalendarViewDayWidth/2-32/2;
        int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2) + kVRGCalendarViewDayHeight-5;
