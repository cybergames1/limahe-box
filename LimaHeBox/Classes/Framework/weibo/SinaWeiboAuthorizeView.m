...
- (void)dealloc
{
    ...
    if ([timeoutTimer isValid]) {
        ...
        [timeoutTimer release],timeoutTimer = nil;
    }
    [super dealloc];
}
...
- (void)showWebView
{
    UIWindow* window = [CommonTools keyWindow];
    if (!window)
    {	
...
- (void)show
{
    ...
    timeoutTimer = [[NSTimer scheduledTimerWithTimeInterval:kTimeOutIntervel target:self selector:@selector(requestTimeout) userInfo:nil repeats:NO] retain];
...
- (void) requestTimeout
{
    ...
    NSString *error = @"请求超时";
    NSString *error_uri = @"error_uri";
    NSString *error_description = @"绑定账号请求超时";
